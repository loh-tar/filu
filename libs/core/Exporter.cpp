  ...
  if(mCmd->wantHelp())
  {
    ...
    mCmd->inOptBrief("symbols", "<Type>.."
                    , tr("Only these kind of symbols. But note that <Type> can not be your "
                        ...
                        "to note also 'symbols' explicit to export as data type"));
...
void Exporter::cmdAll()
{
  ...
  if(mCmd->isMissingParms())
  {
    ...
    mCmd->printForInst("--into mybackup.imp");
...
void Exporter::cmdData()
{
  ...
  if(mCmd->isMissingParms())
  {
    ...
    mCmd->prin4Comment(tr("Depot dates include all historic data relating to depots but no old order data, "
                          "only active orders."));
    mCmd->prin4Comment(tr("Exported is: Depot, Positions, AccountPostings, Orders"));
    mCmd->prin4Comment(tr("Not exported is: Broker, FIs, Symbols, EodBars, Markets"));
    mCmd->prin4Comment(tr("Therfore is it currently not possible to restore the full depot and history "
                          "with only to to give 'depots' as data parameter."));
    ...
    mCmd->printForInst("depots --owner Me --into my-depot-backup.imp --verbose 2");
    mCmd->aided();
    return;
  }
...
//     mExport.removeAll();
  ...
  mOLine << "* Visit http://filu.sourceforge.net";
  mOLine << "*";
  mOLine << "* Command was:";
